...
class SshSessionContractTests(unittest.TestCase):
    ...
    def test_interactive_shell_registers_and_services_channel_request(self):
        ...
        self.assertIn('output.prompt()', source)
        ...
        self.assertIn("void prompt() { send_text(\"esp32shell>\", false); }", source)
        self.assertIn("void raw(const char* text) { send_text(text, false); }", source)
        self.assertIn("output.prompt();", source)
        self.assertIn('output.raw("\\r\\n")', source)
        self.assertIn('output.raw("\\b \\b")', source)
        self.assertIn("char echoed[2] = {ch, '\\0'}", source)
        self.assertNotIn('output.line("esp32shell>");', source)
